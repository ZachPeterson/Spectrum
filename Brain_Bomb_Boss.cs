    ...
    class Brain_Bomb_Boss : Brain
    {
        ...
        public override void UpdateBrain(float DT)
        {
            ...
            if (mCrossValue.Z > -0.1f && mCrossValue.Z < 0.1f)
            {
                ...
                {
                    mShip.ActivateAbility(AbilityType.FireMultipleBullets, mShip.mRotation, false);
                    mShootTracker = 0;
                }
            }

            if(nearDeath)
                mShip.ActivateAbility(AbilityType.DeployBombGrunts, mShip.mRotation, true);
            else
                mShip.ActivateAbility(AbilityType.DeployBombGrunts, mShip.mRotation);
